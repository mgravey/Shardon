...
def create_app(runtime_label: str) -> FastAPI:
            ...
            "choices": [
                {
                    ...
                    "message": {
                        "role": "assistant",
                        "content": f"[{runtime_label}] Shardon demo reply: {prompt[:120]}",
                    },
                    "finish_reason": "stop",
                }
            ],
    ...
    async def completion(payload: dict[str, object]) -> dict[str, object]:
        ...
        return {
            ...
            "model": payload.get("model", os.environ.get("SHARDON_API_MODEL_NAME", "demo-chat")),
            "choices": [{"index": 0, "text": f"[{runtime_label}] completion for {prompt}", "finish_reason": "stop"}],
        }
    ...
    async def embeddings(payload: dict[str, object]) -> dict[str, object]:
        ...
        return {
            "object": "list",
            "data": [
                {"object": "embedding", "index": index, "embedding": [float(index), 0.1, 0.2, 0.3]}
